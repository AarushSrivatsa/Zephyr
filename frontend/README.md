...
```
html/
  index.html              Landing page (nav, hero + inline workflow, pricing, footer)
...
ts/
  ...
  landing.ts                 Entry point for index.html — connect buttons + workflow animation
...
tsconfig.json               rootDir: ts, outDir: js
package.json
```
...
```bash
npm install
npm run build      # tsc: ts/*.ts -> js/*.js, tailwindcss: css/tailwind.css -> css/style.css
```
...
```bash
npm run serve        # http-server on http://localhost:5500 — open /html/index.html
# or
python3 -m http.server 5500
```

All CSS/JS references in the HTML use root-absolute paths (`/css/style.css`,
`/js/dashboard.js`), so they resolve correctly whether a page is served
straight from this folder (dev) or copied flat into the backend's
`frontend/` directory (prod — see the Dockerfile).
...
markup `dashboard.ts` and `landing.ts` generate for rule cards, toasts, and
the workflow chips). `tailwind.config.js` holds the design tokens — colors,
fonts, shadows, radii — so a rebrand only touches that one file.
`css/tailwind.css` is just the three `@tailwind` directives plus a couple of
true base-layer rules (font smoothing, the focus ring, reduced-motion) that
apply globally and don't belong on individual elements.
...
## How auth + routing work here

- **Signed-in visitors never see the landing page.** An inline script at
  the very top of `index.html`'s `<head>` reads the same `localStorage` key
  `api.ts` uses and calls `location.replace("/dashboard")` before anything
  paints — no flash of the marketing page, no extra click on a "Dashboard"
  button.
- **Signed-out visitors land on the marketing page** and use "Login with
  Instagram", which sends the browser to `GET {API_BASE_URL}/user/login`.
- Instagram redirects back to `login-callback.html?code=...`.
- `callback.ts` calls `GET /user/instagram_callback?code=...`, stores the
  returned tokens in `localStorage`, and redirects to `/dashboard`.
- `api.ts` attaches `Authorization: Bearer <access_token>` to every call and
  silently refreshes once via `POST /user/refresh` on a 401.
